...
export default function DadosBI(){
 return <SolutionPageLayout
  ...
  features={features}
 >
  <section className="py-24 lg:py-32 bg-white"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><AnimatedSection className="grid grid-cols-1 lg:grid-cols-[.8fr_1.2fr] gap-14"><div><div className="font-mono text-[11px] tracking-[.2em] uppercase text-[#C4191F] mb-4">Da mídia à receita</div><h2 className="font-display font-black text-4xl sm:text-5xl lg:text-6xl tracking-[-.045em] leading-[.94]">O clique é só o começo da história.</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-border border border-border">{["Impressão → visita","Visita → lead","Lead → oportunidade","Oportunidade → venda","Venda → receita","Receita → retorno"].map((x,i)=><div key={x} className="bg-white p-7 min-h-[150px] flex flex-col justify-between"><span className="font-mono text-xs text-[#C4191F]">{String(i+1).padStart(2,"0")}</span><strong className="font-display font-black text-xl">{x}</strong></div>)}</div></AnimatedSection></div></section>
